...
uint16_t valTemp = 0xFFFF, valHum = 0xFFFF;
HTU20D htu20d;
...
static void taskPeriodicSend(void *) {
  ...
  f->len = sprintf(
    (char *) f->buf,
    "\"TempC\":%u.%02u,\"Hum\":%u.%02u",
    valTemp / 100, valTemp % 100, valHum / 100, valHum % 100
  );
...
static void eventLoRaWANJoin(
  ...
  if (joinedNwkSKey && joinedAppSKey) {
    ...
    if (joined) {
      ...
      lw.beginJoining(NULL, NULL, NULL);
    }
  } else {
...
void setup() {
  ...
  pinMode(GPIO5, INPUT);
  attachInterrupt(GPIO5, eventButtonPressed, FALLING);
  ...
  timerSense.onFired(taskSense, NULL);
  timerSense.startPeriodic(1000);
}
